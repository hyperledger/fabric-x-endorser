...
licensecheck:
	...
	@find . -name '*.go' -not -path '*/vendor/*' | xargs addlicense -check || (echo "Missing license headers"; exit 1)

.PHONY: gofmt
gofmt:
	@echo Running gofmt
	@{ \
	OUTPUT="$$(find . -name '*.go' -not -path '*/vendor/*' | xargs gofmt -l -s || true)"; \
	if [ -n "$$OUTPUT" ]; then \
...
goimports:
	...
	@{ \
	OUTPUT="$$(find . -name '*.go' -not -path '*/vendor/*' ! -name '*.pb.go' | xargs goimports -l)"; \
	if [ -n "$$OUTPUT" ]; then \
...
misspell:
	...
	@{ \
	OUTPUT="$$(find . -name '*.go' -not -path '*/vendor/*' | xargs misspell || true)"; \
	if [ -n "$$OUTPUT" ]; then \
